...
#     aruco_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_1000) #이거 사이즈 6x6 -> 4x4
...
def detect_markers(image, camera_matrix, dist_coeffs, marker_size):
    ...
    if ids is not None:
        ...
        for i, marker_id in enumerate(ids.flatten()):
            ...
            distance = np.linalg.norm(tvecs[i]) *0.767 -0.076 # 거리 Z
            fx = camera_matrix[0, 0]
            fy = camera_matrix[1, 1]
            cx = camera_matrix[0, 2] /2
            cy = camera_matrix[1, 2] /2
            print("fx,fy,cx,cz",fx,fy,cx,cy) #525.0 525.0 320.0 240.0

            

            X = (center_u - cx) * distance / fx
            Y = (center_v - cy) * distance / fy
            Z = distance

            # msg_distance.data = corrected_distance*0.767 -0.076
...
class ArucoMarkerDetector(Node):
    ...
    def listener_callback(self, msg):
        ...
        if len(detect_data) == 0:
            ...
            corrected_distance = closest_marker[3]
            ...
            # print(f'marker_pulish={marker_array_msg}')
